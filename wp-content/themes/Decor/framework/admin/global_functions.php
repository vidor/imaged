	...
	switch ( $value['type'] ) :
		...
		case 'select_slideshow':
		   ...
		   $posts_obj = get_posts($args);
			...
			if($value['id']=='back_slideshow') $output .= '<option value="default">default</option>';
			
			
			
			if($value['id']=='back_gallery' || $value['id']=='back_slideshow'){
			
				if($selected_value=='off') $output .= '<option selected="selected" value="off">off</option>';
				else $output .= '<option value="off">off</option>';
